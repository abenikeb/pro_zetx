  ...
  >
    ...
    <form class="modal-form" @submit.prevent="updateProduct">
      <div class="modal-header">
        <h4 class="text-xl font-bold">Update Product</h4>
      ...
      <div class="pt-4">
          ...
          <div class="flex flex-col">
            ...
            <fieldset class="form-control">
              ...
              >
                ...
                <option
                  v-for="(countryData, idx) in countryNames"
                  :key="idx"
        ...
        <!-- Submit Row -->
        <div class="flex justify-start mb-2">
          <button type="submit" class="green-btn">Update Item</button>
        </div>
        <!-- END - Submit Row -->
      </div>
      <!-- END - Container -->
...
export default Vue.extend({
  ...
  */
  ...
  methods: {
    ...
    async init() {
      if (typeof this.productId === 'undefined') {
        this.$modal.hide('update-product')
        return
      }

      this.toggleLoading(true)
      await this.$store.dispatch('market/fetchCategories') // update categories
      await this.$store.dispatch('market/fetchTags') // update tags
    ...
    async initModal() {
      ...
      for (const tag of product.tags!)
        this.tags.push({
          name: tag,
          value: tag.toLowerCase().replaceAll(' ', ''),
        })
    ...
    async updateProduct() {
      ...
      await this.$store.dispatch('products/fetchAllProducts') // Update products after product created
